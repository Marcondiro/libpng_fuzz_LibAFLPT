...
// https://raw.githubusercontent.com/chromium/chromium/refs/heads/main/testing/libfuzzer/fuzzers/libpng_read_fuzzer.cc
// All credits go to The Chromium Authors, LICENSE can be found at
...
static void my_png_error(png_structp png_ptr, png_const_charp) {
    // libpng expects this function *not* to return.
    // You can longjmp using the png_ptr's jump buffer:
    png_longjmp(png_ptr, 1);
}

static void my_png_warning(png_structp, png_const_charp) {}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  png_structp png_ptr = png_create_read_struct
    (PNG_LIBPNG_VER_STRING, nullptr, my_png_error, my_png_warning);
  assert(png_ptr);
